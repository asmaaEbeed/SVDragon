...
class ReportController extends Controller
{
            ...
            $query = PurchaseLine::join(
                ...
                't.id'
                    ...
                    ->select(
                        ...
                        'purchase_lines.avg_purchase_price as unit_purchase_price',
    ...
    public function getProfit($by = null)
    {
        ...
        $query->select(DB::raw('SUM(IF (TSPL.id IS NULL AND P.type="combo", ( 
            SELECT Sum((tspl2.quantity - tspl2.qty_returned) * (tsl.unit_price_inc_tax - pl2.avg_purchase_price)) AS total
                ...
                (TSPL.quantity - TSPL.qty_returned) * (transaction_sell_lines.unit_price_inc_tax - PL.avg_purchase_price)) )) AS gross_profit')
            );
        ...
        if (request()->ajax()) {
            $query = TransactionSellLinesPurchaseLines::leftJoin('transaction_sell_lines 
                ...
                ->select(
                    ...
                    'PL.avg_purchase_price as purchase_price',
    ...
    public function getStockValue()
    {
        ...
        //Get Closing stock
        $closing_stock_by_pp = $this->transactionUtil->getOpeningClosingStockForReport(
            $business_id,
            $end_date,
            $location_id,
            false,
        ...
        $closing_stock_by_sp = $this->transactionUtil->getOpeningClosingStockForReport(
            $business_id,
            $end_date,
            $location_id,
            false,
