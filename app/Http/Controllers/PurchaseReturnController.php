...
class PurchaseReturnController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $purchase_total_paid = $this->transactionUtil->getTotalPaid($purchase->id);
            if($purchase->payment_status == 'paid' &&  $purchase->final_total - $purchase_total_paid == 0)
            {
                $transaction_before = $return_transaction->replicate();
                $inputs['amount'] = $return_transaction->final_total;
                $inputs['paid_on'] = \Carbon::now();
                $inputs['transaction_id'] = $return_transaction->id;
                $inputs['amount'] = $this->transactionUtil->num_uf($inputs['amount']);
                $inputs['created_by'] = auth()->user()->id;
                $inputs['payment_for'] = $return_transaction->contact_id;
                $inputs['card_type'] = 'credit';
                $inputs['is_advance'] = 1 ;
                $prefix_type = 'purchase_payment';
                $ref_count = $this->transactionUtil->setAndGetReferenceCount($prefix_type);
                $inputs['method'] = 'advance';
                ...
                $payment_amount = $return_transaction->final_total;
                $contact_balance = !empty($return_transaction->contact) ? $return_transaction->contact->balance : 0;
                ...
                if (!empty($inputs['amount'])) {
                    ...
                    $inputs['transaction_type'] = $return_transaction->type;
                    event(new TransactionPaymentAdded($tp, $inputs));
                }
                //update payment status
                $payment_status = $this->transactionUtil->updatePaymentStatus($return_transaction->id, $return_transaction->final_total);
                $return_transaction->payment_status = $payment_status;

                $this->transactionUtil->activityLog($return_transaction, 'payment_edited', $transaction_before);
            }elseif( $purchase->payment_status == 'partial')
            {
                // if return final total = payment due
                if($purchase->final_total - $purchase_total_paid == $return_transaction->final_total)
                {
                    $return_transaction->payment_status = 'paid';
                    $return_transaction->save();
                    $purchase->payment_status = 'paid';
                    $purchase->save();

                }elseif($purchase->final_total - $purchase_total_paid < $return_transaction->final_total)
                {
                    $transaction_before = $return_transaction->replicate();
                    $inputs['amount'] =$return_transaction->final_total - ($purchase->final_total - $purchase_total_paid);
                    $inputs['paid_on'] = \Carbon::now();
                    $inputs['transaction_id'] = $return_transaction->id;
                    $inputs['amount'] = $this->transactionUtil->num_uf($inputs['amount']);
                    $inputs['created_by'] = auth()->user()->id;
                ...
                if($purchase->final_total - $return_transaction->final_total == 0 )
                {
                    $purchase->payment_status = 'paid';
                }
                $purchase->save();
