...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $combo_variations = [],$su_sell_price = null)
    {
        ...
        $variation_data = [
            ...
            'su_sell_price' => $su_sell_price==null?null:$this->num_uf($su_sell_price),
            'su_sell_price_inc_tax' => $su_sell_price==null?null:$this->num_uf($su_sell_price),
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        ...
        if ($product->enable_stock == 1) {
            ...
            if ($product->main_product_id) {
                $main_product_id = $product->main_product_id;
                $main_variation = Variation::where('product_id', $main_product_id)->firstOrFail();
                $details = VariationLocationDetails::where('variation_id', $main_variation->id)
            ...
            if (empty($details) && $product->main_product_id == null) {
    ...
    {
        // check main product qty

        $variation_check = Variation::where('id', $variation_id)->first()->product;
        if ($variation_check->main_product_id) {
            $main_variations = Product::where('id', $variation_check->main_product_id)->first()->variations;
            $main_variation_id = $main_variations->first()->id;
            $main_variation_location_detail = VariationLocationDetails::where('location_id', $location_id)
                ->where('variation_id', $main_variation_id)->where('product_id', $variation_check->main_product_id)->firstOrFail();
            $main_qty_avail = $main_variation_location_detail->qty_available;
        }
        // end check
        ...
        $product = $query->select(
            ...
            'variations.su_sell_price',
            'variations.su_sell_price_inc_tax',
            'variations.avg_purchase_cost',
        ...
        if ($variation_check->main_product_id) {
            if ($main_qty_avail > 0) {
                $product->qty_available = $main_qty_avail;
            } else {
                $product = [];
            }
        }
     ...
     * @return void
     */
    public function updateProductFromPurchase($variation_data, $has_parent_product = false, $old_variation_id)
    {
        $variation_details = Variation::where('id', $variation_data['variation_id'])
            ->with(['product', 'product.product_tax'])
            ->first();
        $old_variation_details = Variation::where('id', $old_variation_id)
            ->with(['product', 'product.product_tax'])
            ->first();
        $tax_rate = 0;
        if ($old_variation_id == $variation_data['variation_id']) {
            ...
            if (!isset($variation_data['su_sell_price_inc_tax'])) {
                $variation_data['su_sell_price_inc_tax'] = $variation_details->su_sell_price_inc_tax;
            }
            if (($variation_details->default_purchase_price != $variation_data['pp_without_discount']) ||
                ...
                ($variation_details->avg_purchase_cost != $variation_data['avg_purchase_cost']) ||
                ($variation_details->su_sell_price_inc_tax != $variation_data['su_sell_price_inc_tax'])
            ) {
                //Set default purchase price exc. tax
                $variation_details->default_purchase_price = $variation_data['pp_without_discount'];
                //set avg purchase cost
                $variation_details->avg_purchase_cost = $variation_data['avg_purchase_cost'];
                //set last purchase cost
                $variation_details->last_purchase_cost = $variation_data['last_purchase_cost'];
                ...
                //Set default subunit sell price inc. tax
                $variation_details->su_sell_price_inc_tax = $variation_data['su_sell_price_inc_tax'];

                //set sell price inc. tax
                // $this->calc_percentage_base($variation_details->su_sell_price?:00, $tax_rate);
                $variation_details->su_sell_price = $this->calc_percentage_base($variation_details->su_sell_price_inc_tax, $tax_rate) ;
            ...
            if (!isset($variation_data['su_sell_price_inc_tax'])) {
                $variation_data['su_sell_price_inc_tax'] = $old_variation_details->su_sell_price_inc_tax;
            }
            if (($variation_details->default_purchase_price != $variation_data['pp_without_discount']) ||
                ($variation_details->sell_price_inc_tax != $variation_data['sell_price_inc_tax']) ||
                ($variation_details->avg_purchase_cost != $variation_data['avg_purchase_cost']) ||
                ($old_variation_details->su_sell_price_inc_tax != $variation_data['su_sell_price_inc_tax'])
            ) {
                //Set default purchase price exc. tax
                $variation_details->default_purchase_price = $variation_data['pp_without_discount'];
                //set avg purchase cost
                ...
                //set sell price inc. tax
                $old_variation_details->su_sell_price =  $this->calc_percentage_base($old_variation_details->su_sell_price_inc_tax, $tax_rate) ;

                //set profit margin
                $variation_details->profit_percent = $this->get_percent($variation_details->default_purchase_price, $variation_details->default_sell_price);
                $old_variation_details->save();
                $variation_details->save();
            }
        }
    }
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                $main_product_value = Product::findOrFail($data['product_id']);
                $product_id_check = $main_product_value->main_product_id ?: $data['product_id'];
                if ($main_product_value->main_product_id) {
                    $main_variation = Variation::where('product_id', $main_product_value->main_product_id)->first();
                }
                $main_variation_id = $main_product_value->main_product_id ? $main_variation->id : $data['variation_id'];
                $this->updateProductStock($before_status, $transaction, $product_id_check, $main_variation_id, $new_quantity, $purchase_line->quantity, $currency_details);
            } else {
                ...
                $main_product_value_id = $main_product_value->main_product_id ?: $data['product_id'];
                $main_variation_id = $main_product_value->main_product_id ? $main_variation->id : $data['variation_id'];
                $purchase_line = new PurchaseLine();
                $purchase_line->product_id = $main_product_value_id;
                // assign main product variation to purchase line variation
                $purchase_line->variation_id = $main_variation_id;
                $new_line_price_without_disc =  ($this->num_uf($data['pp_without_discount'], $currency_details) * $exchange_rate) / $multiplier;
                $new_line_price_with_disc =  ($this->num_uf($data['purchase_price'], $currency_details) * $exchange_rate) / $multiplier;
                $latest_purchase_line = PurchaseLine::where('product_id', $main_product_value_id)->where('variation_id', $main_variation_id)->orderBy('id', 'DESC')->first();
                $old_stock_unit_cost = $latest_purchase_line ? ($latest_purchase_line->avg_purchase_price ?: $latest_purchase_line->pp_without_discount) : 0;
                $purchase_line->avg_purchase_price = $this->calculateAvgPurchasePrice($main_variation_id, $old_stock_unit_cost, $main_product_value_id, $new_line_price_without_disc, $new_quantity, $transaction->location_id, $transaction->business_id);
                $purchase_line->avg_purchase_price_with_disc = $this->calculateAvgPurchasePrice($main_variation_id, $old_stock_unit_cost, $main_product_value_id, $new_line_price_with_disc, $new_quantity, $transaction->location_id, $transaction->business_id);
                //Increase quantity only if status is received
                if ($transaction->status == 'received') {
                    $this->updateProductQuantity($transaction->location_id, $main_product_value_id, $main_variation_id, $new_quantity_f, 0, $currency_details);
                }
            }
            ...
            if ($enable_product_editing == 1) {
                ...
                if (isset($data['su_sell_price'])) {
                    $variation_data['su_sell_price_inc_tax'] = ($this->num_uf($data['su_sell_price'], $currency_details));
                }

                $variation_data['pp_without_discount'] = $new_line_price_without_disc == $purchase_line->avg_purchase_price ? $new_line_price_without_disc : $purchase_line->avg_purchase_price;
                $variation_data['variation_id'] = $purchase_line->variation_id;
                $variation_data['avg_purchase_cost'] = $purchase_line->avg_purchase_price;
                $variation_data['last_purchase_cost'] = $new_line_price_without_disc;
                //todo add avg cost data
                $this->updateProductFromPurchase($variation_data, $has_parent_product, $data['variation_id']);
            }
    ...
    /**
     *calculate average purchase price for average cost for business stock accounting 
     *
     ...
     * @return float
     */
    public function calculateAvgPurchasePrice($variation_id, $old_stock_unit_cost, $product_id, $purchase_price, $new_quantity, $location_id, $business_id)
    {
        if (session()->has('business') && request()->session()->get('business')['accounting_method'] == 'avco') {
            $current_stored_stock = $this->getCurrentStock($variation_id, $location_id);

            if ($current_stored_stock > 0 || $old_stock_unit_cost > 0) {
                $new_product_qty = $new_quantity;
                $new_product_cost = $purchase_price;
                $avg_unit_cost = (($new_product_qty * $new_product_cost) + ($old_stock_unit_cost * $current_stored_stock)) / ($new_product_qty + $current_stored_stock);
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ...
            ->leftjoin('units as SU', 'products.sub_unit_ids', '=', 'U.id')
            ->leftjoin(
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        $query->whereNull('p.main_product_id');
        ...
        $products = $query->select(
            ...
            DB::raw("(SELECT SUM( COALESCE(pl.quantity - ($pl_query_string), 0) * variations.avg_purchase_cost) FROM transactions 
