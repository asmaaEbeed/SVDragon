...
class TransactionUtil extends Util
{
        ...
        if ($due_transactions->count()) {
            foreach ($due_transactions as $transaction) {
                ...
                if ($total_amount > 0) {
                    ...
                    $returned_amount = $return_transaction? $return_transaction->final_total : 0;
                    $due = $transaction->final_total - $total_paid - $returned_amount;
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            if($product->main_product_id){
                $line->product_id = $product->main_product_id;
                $main_variation = Variation::where('product_id',$product->main_product_id)->select('id')->first();
                $line->variation_id = $main_variation->id;
            }
     ...
     * @return float
     */
    public function getOpeningClosingStockForReport($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [])
    {
        $query = PurchaseLine::join(
            'transactions as purchase',
            'purchase_lines.transaction_id',
        ...
        $price_query_part = "(purchase_lines.avg_purchase_price + 
                            COALESCE(purchase_lines.item_tax, 0))";
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        ...
        $gross_profit_obj = $query->select(DB::raw('SUM( 
                        (transaction_sell_lines_purchase_lines.quantity - transaction_sell_lines_purchase_lines.qty_returned) * (SL.unit_price_inc_tax - IFNULL(PL.avg_purchase_price, v.avg_purchase_cost) ) ) as gross_profit'))
            ->first();
     ...
     *
     */
    public function getContactDueDetails($contact_id , $start = null, $end = null)
    {
        //Get sum of totals before start date
        $previous_transaction_sums = $this->__transactionQuery($contact_id)
            ->select(
                DB::raw("SUM(IF(type = 'purchase', final_total, 0)) as total_purchase"),
                DB::raw("SUM(IF(type = 'sell' AND status = 'final', final_total, 0)) as total_invoice"),
                DB::raw("SUM(IF(type = 'sell_return', final_total, 0)) as total_sell_return"),
        ...
        //Get payment totals before start date
        $prev_payments = $this->__paymentQuery($contact_id)
            ->select('transaction_payments.*', 'bl.name as location_name', 't.type as transaction_type')
            ->get();
        ...
        //Get transaction totals between dates
        $transactions = $this->__transactionQuery($contact_id)
            ->with(['location'])->get();
        $transaction_types = Transaction::transactionTypes();
        $ledger = [];
        ...
        $start_date = $start?$this->format_date($start):null;
        $end_date = $end?$this->format_date($end):null;
     ...
     *
     */
    private function __transactionQuery($contact_id, $start = null, $end = null)
    {
     ...
     *
     */
    private function __paymentQuery($contact_id, $start = null, $end = null)
    {
        $business_id = request()->session()->get('user.business_id');
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        //Get Opening stock
        $opening_stock = $this->getOpeningClosingStockForReport($business_id, $day_before_start_date, $location_id, true, false, $filters);

        $opening_stock_by_sp = $this->getOpeningClosingStockForReport($business_id, $day_before_start_date, $location_id, true, true, $filters);

        //Get Closing stock
        $closing_stock = $this->getOpeningClosingStockForReport(
            $business_id,
            $end_date,
            $location_id,
            false,
        ...
        $closing_stock_by_sp = $this->getOpeningClosingStockForReport(
            $business_id,
            $end_date,
            $location_id,
            false,
