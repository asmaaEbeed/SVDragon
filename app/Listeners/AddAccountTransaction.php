...
class AddAccountTransaction
{
    ...
    public function handle(TransactionPaymentAdded $event)
    {
        ...
        if ($event->transactionPayment->method == 'advance') {
            $balance_calculation  = 'deduct';
            $event->formInput['transaction_type'] == 'purchase_return'?$balance_calculation = 'add':$balance_calculation;
            $this->transactionUtil->updateContactBalance($event->transactionPayment->payment_for, $event->transactionPayment->amount, $balance_calculation);
        }
