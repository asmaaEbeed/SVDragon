...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		@php
			...
			$sub_unit_price_inc_tax = $product->su_sell_price_inc_tax;
			...
			if($hide_tax == 'hide'){
				...
				$sub_unit_price_inc_tax = $product->su_sell_price;
			}
	...
	<td>
		...
		<div class="input-group input-number">
			<span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-down inc-dec-btn"><i class="fa fa-minus text-danger"></i></button></span>
		<input type="text" data-min="1" 
			...
			@endif 
		>
		<span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-up inc-dec-btn"><i class="fa fa-plus text-success"></i></button></span>
		</div>
		...
		<input type="hidden" class="hidden_base_unit_sell_price" value="{{ $product->su_sell_price ?($product->su_sell_price/$value['multiplier']):($product->default_sell_price / $multiplier)}}">
	...
	<input type="hidden" name="products[{{$row_count}}][product_unit_id]" value="{{$product->unit_id}}">
		@if(count($sub_units) > 0)
			<select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit pos-unit-col">
                @foreach($sub_units as $key => $value)
                    <option value="{{$key}}" data-multiplier="{{$value['multiplier']}}" data-unit_name="{{$value['name']}}" data-allow_decimal="{{$value['allow_decimal']}}" @if(!empty($product->sub_unit_id) && $product->sub_unit_id == $key) selected @endif>
                        {{$value['name']}}
                    </option>
	...
	<td class="text-center">
		...
		@endphp
		<input type="{{$subtotal_type}}" class="form-control pos-subtotal pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{@num_format($product->quantity_ordered*$unit_price_inc_tax )}}">
	...
	<td class="text-center v-center">
		<i class="fa fa-times text-danger pos_remove_row cursor-pointer pos-table-remove-prod" aria-hidden="true"></i>
	</td>
</tr>