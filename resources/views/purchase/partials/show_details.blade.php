...
<div class="modal-body">
  ...
  <div class="row">
    @if(!empty($purchase->type == 'purchase'))
    <div class="col-sm-12 col-xs-12">
      <h4>{{ __('sale.purchase_return_info') }}:</h4>
    </div>
    <div class="col-md-6 col-sm-12 col-xs-12">
      <div class="table-responsive">
        <table class="table">
          <tr class="bg-green">
            ...
            <th>{{ __('sale.amount') }}</th>
            <th>{{ __('sale.payment_mode') }}</th>
            <th>{{ __('sale.advance_amount') }}</th>
          </tr>
          @if($purchase_return)
         
            <tr>
              <td>#</td>
              <td>{{ @format_date($purchase_return->transaction_date ) }}</td>
              <td>{{ $purchase_return->ref_no }}</td>
              <td><span class="display_currency" data-currency_symbol="true">{{ $purchase_return->final_total }}</span></td>
              <td>{{  !$purchase_return->payment_lines->isEmpty()? ($payment_methods[$purchase_return->payment_lines[0]->method] ?? ''):'' }}</td>
              <td>{{ !$purchase_return->payment_lines->isEmpty()? $purchase_return->payment_lines[0]->amount : '--'}}</td>
            </tr>
          @else
            <tr>
