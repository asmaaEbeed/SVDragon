...
<div class="table-responsive">
    <table class="table table-bordered add-product-price-table table-condensed {{$class}}">
        ...
        <tr>
          <td>
            <div class="col-sm-6">
              @if(session('business.enable_tax_box'))
              {!! Form::label('single_dpp', trans('product.exc_of_tax') . ':*') !!}
              @else
              {!! Form::label('single_dpp','  ') !!}
              @endif
              {!! Form::text('single_dpp', $default, ['class' => 'form-control input-sm dpp input_number', 'placeholder' => ' ', 'required']); !!}
            </div>

            <div class="col-sm-6 @if(!session('business.enable_tax_box'))hide @endif">
