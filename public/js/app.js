$(document).ready(function() {
    ...
    $(document).on('change', '.payment_types_dropdown', function() {
        ...
        account_dropdown = $('#pay_contact_due_form #account_id');
        amount_element = payment_row.find('#total_amount');
    ...
    $(document).on('click', 'a.pay_purchase_due, a.pay_sale_due', function(e) {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                account_select = payment_row.find('.account_select');
                if (payment_type == 'advance') {
                    max_value = $('#advance_balance').val();
                    msg = $('#advance_balance').data('error-msg');
                    amount_element.rules('add', {
                ...
                $('#main_amount').on('input',function() {
                    ...
                    var sale_due = parseInt($('#sale_due').text().replace(/[^0-9.]/g, ""));
                    var main_amount = $('#main_amount').val();
                    var percentage_amount = main_amount*comm_percentage/(1-comm_percentage);
                    $('#percentage_amount').val(percentage_amount.toFixed(2));
                    var total_amount = (parseFloat(main_amount) + parseFloat(percentage_amount)).toFixed(2)
                    $('#total_amount').val(total_amount>0 ? total_amount : 0 );
                });
            },
        });
    });
...
/// Add new panel theme added by Asmaa
...
$('.panel--btn').on('click', function () {
    if ($('.theme-panel').hasClass('theme-panel-open')) {
        $('.theme-panel').removeClass('theme-panel-open');
        $('.theme-panel').addClass('theme-panel-close');
    } else {
        $('.theme-panel').addClass('theme-panel-open');
        $('.theme-panel').removeClass('theme-panel-close');
    }
});
$('.theme-panel-close-btn').on('click', function () {
    $('.theme-panel').removeClass('theme-panel-open');
    $('.theme-panel').addClass('theme-panel-close');
});

$('.theme-bg-small').on('click', function () {
...
$('.theme-bg-small').on('click', function () {
    if (
        $(".switcher input[name='theme-color']:checked").val() !== '' &&
        !$('.theme-bg-small').hasClass('theme-bg-small-disabled')
    ) {
        submitThemeFrm();
    }
});

function submitThemeFrm() {
    // This class added to prevent multi click until complete hit server
   //  $('.theme-bg-small').addClass('theme-bg-small-disabled');
    $('form[name=theme_color_pnl_frm').submit();

    var data = $('form#theme_color_pnl_frm').serialize();
    console.log(data);
    // var themeColor = $(".switcher input[name='theme-color']:checked").val();
    $.ajax({
        method: 'POST',
        url: $('form#theme_color_pnl_frm').attr('action'),
        dataType: 'json',
        data: data,
        success: function (result) {
            if (result.success == true) {
            ...
            $('.theme-bg-small').removeClass('theme-bg-small-disabled');
        },
    });
}
...
//// Add new panel theme added by Asmaa
//remove sidebar-collapse at large screen by Asmaa.
...
//End remove sidebar-collapse at large screen by Asmaa
//close treeview menu when click on any part of page added by Asmaa
...
// end close treeview menu when click on any part of page by Asmaa
