...
$(document).ready(function () {
    ...
    //If customize sell price according to unit multiplier
    $(document).on('change','#single_dsp',function(){
        let base_unit_price= $('#single_dsp').val();
        let base_unit_multiplier2 = $('input[name="base_unit_multiplier2"]').val()>=0?$('input[name="base_unit_multiplier2"]').val():0;
        let base_unit_multiplier3 = $('input[name="base_unit_multiplier3"]').val()>=0?$('input[name="base_unit_multiplier3"]').val():0;
        let base_unit_multiplier4 = $('input[name="base_unit_multiplier4"]').val()>=0?$('input[name="base_unit_multiplier4"]').val():0;
        let su_sell_price2 =$('#su_sell_price2');
        let su_sell_price3 =$('#su_sell_price3');
        let su_sell_price4 =$('#su_sell_price4');
        if (base_unit_price) {
            su_sell_price2.val(base_unit_price * base_unit_multiplier2 );
            su_sell_price3.val(base_unit_price * base_unit_multiplier3 );
            su_sell_price4.val(base_unit_price * base_unit_multiplier4 );
        }
    });
    $(document).on('change','input[name="base_unit_multiplier2"]',function () {
        let base_unit_price= $('#single_dsp').val();
        let base_unit_multiplier2 = $('input[name="base_unit_multiplier2"]').val()>=0?$('input[name="base_unit_multiplier2"]').val():0;
        $('#su_sell_price2').val(base_unit_price * base_unit_multiplier2 );
    });
    $(document).on('change','input[name="base_unit_multiplier3"]',function () {
        let base_unit_price= $('#single_dsp').val();
        let base_unit_multiplier3 = $('input[name="base_unit_multiplier3"]').val()>=0?$('input[name="base_unit_multiplier3"]').val():0;
        $('#su_sell_price3').val(base_unit_price * base_unit_multiplier3);
    });
    $(document).on('change','input[name="base_unit_multiplier4"]',function () {
        let base_unit_price= $('#single_dsp').val();
        let base_unit_multiplier4 = $('input[name="base_unit_multiplier4"]').val()>=0?$('input[name="base_unit_multiplier4"]').val():0;
        $('#su_sell_price4').val(base_unit_price * base_unit_multiplier4 );
    }); 
    ...
    $('#sku1').on('blur',function(e){
        ...
        $.ajax({
            ...
            success: function(result){
                if(result=='false'){
                    $('#sku1_alert').removeAttr('hidden');                
                }else{
                    $('#sku1_alert').attr('hidden',true);
                }
            }
        });
    });
    $('#sku2').on('blur',function(e){
        ...
        $.ajax({
            ...
            success: function(result){
                if(result=='false'){
                    $('#sku2_alert').removeAttr('hidden');                
                }else{
                    $('#sku2_alert').attr('hidden',true);
                }
            }
        });
    });
    $('#sku3').on('blur',function(e){
        ...
        $.ajax({
            ...
            success: function(result){
                if(result=='false'){
                    $('#sku3_alert').removeAttr('hidden');                
                }else{
                    $('#sku3_alert').attr('hidden',true);
                }
            }
        });
    });
    $('#sku4').on('blur',function(e){
        // alert($('#sku1').val())
        $.ajax({
            url: '/products/check_product_sku',
            type: 'post',
            ...
            success: function(result){
                if(result=='false'){
                    $('#sku4_alert').removeAttr('hidden');                
                }else{
                    $('#sku4_alert').attr('hidden',true);
                }
            }
        });
    });
    //check nmae duplication with every unit name
    $('[name="actual_name1"]').on('blur',function(e){
        // alert($('#sku1').val())
        $.ajax({
            ...
            data: {
                name: function () {
                    ...
                    var unit_name = $('[name="actual_name1"]').val();
            ...
            success: function(result){
                if(result=='false'){
                    $('#name_alert').removeAttr('hidden');                
                }else{
                    $('#name_alert').attr('hidden',true);
                }
            }
        });
    });
    $('[name="actual_name2"]').on('blur',function(e){
        // alert($('#sku1').val())
        $.ajax({
            url: '/products/check_product_name',
            type: 'post',
            data: {
                name: function () {
                    var base_name = $('#name').val();
                    var unit_name = $('[name="actual_name2"]').val();
                    return base_name+'('+unit_name+')';
                },
            },
            dataType: 'html',
    ...
    $('[name="actual_name3"]').on('blur',function(e){
        // alert($('#sku1').val())
        $.ajax({
            url: '/products/check_product_name',
            type: 'post',
            data: {
                name: function () {
                    var base_name = $('#name').val();
                    var unit_name = $('[name="actual_name3"]').val();
                    return base_name+'('+unit_name+')';
                },
            },
            dataType: 'html',
    ...
    $('[name="actual_name4"]').on('blur',function(e){
        // alert($('#sku1').val())
        $.ajax({
            url: '/products/check_product_name',
            type: 'post',
            data: {
                name: function () {
                    var base_name = $('#name').val();
                    var unit_name = $('[name="actual_name4"]').val();
                    return base_name+'('+unit_name+')';
                },
            },
            dataType: 'html',
        ...
        success: function (result) {
            if (result.success == true) {
                var newOption = new Option(
                    result.data[1].actual_name + '(' + result.data[1].short_name + ')',
                    result.data[1].id,
                    true,
                    true
                );
                // var newSubUnitOption = new Option(
                //     result.data[0].actual_name,
                //     result.data[0].id,
                //     true,
                //     true
                // );
                // Append it to the select
                $('#unit_id option:selected').remove();
                $('#unit_id').append(newOption).trigger('change');
                // 
                $('div.view_modal').modal('hide');
                // $("#sub_unit_ids option[value='"+result.data[1].id+"']").remove();
                // $('#sub_unit_ids').append(newSubUnitOption).trigger('change');
                $('#sub_unit_ids').val(result.data[0].id);
